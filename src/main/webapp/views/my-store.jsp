...
<head>
    ...
    <link rel="stylesheet" href="${contextPath}/resources/css/private-area.css">
    <link rel="stylesheet" href="${contextPath}/resources/css/seller-styles.css">
</head>
<body>
...
<main class="page-component">
    <div class="private-area" style="margin-top: 25px;">
        <div class="private-area-menu">
            ...
            <div class="private-area-list">
                <div class="private-area-list__item"><span class="list-item-active private-area-link" id="dashboard-page-link">Главная</span></div>
                <div class="private-area-list__item"><span class="btn-underline private-area-link" id="orders-page-link">Заказы</span></div>
                <div class="private-area-list__item"><span class="btn-underline private-area-link" id="goods-page-link">Мои товары</span></div>
                <div class="private-area-list__item"><span class="btn-underline private-area-link" id="coupons-page-link">Купоны магазина</span></div>
            </div>

        </div>

        <div class="private-area-content my-dashboard">
            <div class="content-grid">
                <div style="display: flex;">
                    <div class="content-block" style="width: 50%; margin-right: 15px; display: flex; justify-content: center;
                        align-items: center; font-size: 20px; text-align: center;">
                        Выполнено заказов: <br/>
                        ${store.orders.size()}
                    </div>

                    <div class="content-block" style="width: 50%;">
                        <h3 class="content-block__header">Сервис и помощь</h3>
                        <div class="dashboard-help-cont">
                            <div class="trigger-btn-large" style="font-size: unset;text-transform: none; padding: 8px 15px">
                                <a href="">Как начать продавать</a>
                            </div>
                            <div class="trigger-btn-large" style="font-size: unset;text-transform: none; padding: 8px 15px">
                ...
                <div class="content-block" style="width: 100%; margin-top: 45px;">
                    <h3 class="content-block__header">Статистика по продажам</h3>
                    <div style="display: flex; justify-content: space-between;">
                        <div style="border: 1px solid #ccc; padding: 5px 12px; border-radius: 5px">
                            Товаров продано:
                            <div style="text-align: center;">${store.storeStats.goodsSold}</div>
                        </div>

                        <div style="border: 1px solid #ccc; padding: 5px 12px; border-radius: 5px">
                            Количество различных покупателей
                            <div style="text-align: center;">${store.storeStats.diffCustomersCount}</div>
                        </div>

                        <div style="border: 1px solid #ccc; padding: 5px 12px; border-radius: 5px">
                            Количество отзывов:
                            <div style="text-align: center;">${store.orders.size()}</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="private-area-content my-orders hide">
            <h2>Мои заказы</h2>
            <div class="orders-types">
                <div class="order-type order-type-selected">Все заказы</div>
                <div class="order-type btn-underline">Ожидают сборки</div>
                <div class="order-type btn-underline">Доставляются</div>
                <div class="order-type btn-underline">Выполнены</div>
            </div>

            <table class="orders-list gen-table">
                <tr style="background-color: #e8e8e8">
                    <td style="width: 10%; font-size: 17px;">№</td>
                    <td style="width: 15%; font-size: 17px;">Статус</td>
                ...
                <c:forEach var="order" items="${store.orders}">
                    <tr data="${store.id}">
                        ...
                        <td>${order.status}</td>
                        <td>
                            <c:forEach var="good" items="${order.orderGoods}">
                                <div class="order-good-info">${good.name}, ${good.quantity} шт.</div>
                            </c:forEach>
                        </td>
                        <td>${order.sum} руб.</td>
        ...
        <div class="private-area-content my-goods hide">
            <h2>Мои товары</h2>
            <div class="goods-types">
                <div class="good-type order-type-selected">Все товары</div>
                <div class="good-type btn-underline">В продаже</div>
                <div class="good-type btn-underline">Скрытые товары</div>
            </div>

            <div class="goods-controls">
                <div class="goods-searcher">
                    <div class="goods-searcher__search-icon">
                        <svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg" type="submit" aria-label="Поиск" class=""><path d="M9.996 17.991a7.996 7.996 0 117.995-7.995 1 1 0 11-2 0 5.996 5.996 0 10-1.756 4.24l.707-.708 7.115 7.115a1 1 0 11-1.414 1.414l-5.745-5.745a7.967 7.967 0 01-4.902 1.68z" fill="currentColor"></path></svg>
                    </div>

                    <input type="text" class="goods-searcher__input no-border-input" placeholder="Поиск товара по названию или номеру" />
                </div>

                <div class="good-controls-btns">
                    <a href="/store/createStoreGood" class="trigger-btn-large add-good-btn">Добавить товар</a>
                </div>
            </div>


            <div class="goods-list" style="margin-top: 25px">
                <table class="orders-list gen-table">
                    <tr style="background-color: #e8e8e8">
                        <td style="width: 10%; font-size: 17px;">№</td>
                        <td style="width: 15%; font-size: 17px;">Фото</td>
                        <td style="width: 35%; font-size: 17px;">Название</td>
                        <td style="width: 10%; font-size: 17px;">Цена</td>
                        <td style="width: 15%; font-size: 17px;">Скидка</td>
                        <td style="width: 15%; font-size: 17px;">Действия</td>
                    </tr>

                    <c:forEach var="good" items="${store.goods}">
                        <tr>
                            <td>${good.id}</td>
                            <td>
                                <img src="${good.goodImages.get(0).imageSrc}" alt="${good.name}">
                            </td>
                            <td>
                                ${good.name}
                            </td>
                            ...
                            <td>
                                <c:if test="${good.priceBeforeDiscount != null}">
                                    ${ Math.round(100 * (1 - good.price / good.priceBeforeDiscount)) }%
                                </c:if>
                            </td>
                            ...
                            <td class="good-control-btn c-pointer">
                                ...
                                <div class="good-action-panel hide">
                                    <a href="/store/editStoreGood?goodId=${good.id}" class="good-action-panel__btn edit-store-good">Редактировать</a>
        ...
        <div class="private-area-content my-account hide">
            <h2>Редактирование профиля</h2>

            <div class="edit-profile-panel">
                <!-- Добавление основной информации -->
                <h3 style="margin-top: 0;">Основная информация о магазине</h3>

                <form:form action="/store/editStoreInfo" method="POST">
                    <div class="edit-profile-input-cont">
                        <span class="edit-profile-param">Название магазина*</span>
                        <input type="text" name="store-name" class="profile-edit-input" value="${store.name}">
                    </div>

                    <div class="edit-profile-input-cont">
                        <span class="edit-profile-param profile-edit-input">Описание магазина</span>
                        <input type="text" name="store-description" value="${store.description}">
                    </div>
            ...
            <div class="edit-profile-panel" style="margin-top: 25px;">
                <!-- Добавление адреса -->
                <h3 style="margin-top: 0;">Редактирование адреса</h3>
                <form:form action="/store/editStoreAddress" method="POST">
                    <div class="edit-profile-input-cont">
                        <span class="edit-profile-param">Город*</span>
                        <input type="text" name="store-city" class="profile-edit-input" value="${store.storeAddress.city}">
                    </div>

                    <div class="edit-profile-input-cont">
                        <span class="edit-profile-param">Улица</span>
                        <input type="text" name="store-street" class="profile-edit-input" value="${store.storeAddress.street}">
                        <span class="edit-profile-param-icon"></span>
                    </div>

                    <div>
                        <button type="submit" class="register-btn" style="margin-top: 25px; width: 150px; background-color: #800080;">Сохранить</button>
                    </div>
                </form:form>
            </div>
        </div>
...
<script src="${contextPath}/resources/js/store-scripts.js"></script>
