...
<head>
    ...
    <link rel="stylesheet" href="${contextPath}/resources/css/good-card.css">
</head>
<body>
...
<main class="page-component">
    <div class="bread-crumbs content-section">
        ...
        <a href="/goods?categoryName=${good.goodCategories.get(0).name}" class="bread-crumbs__item">${good.goodCategories.get(0).name}</a>
    </div>
    ...
    <div class="brief-info">
        ...
        <div style="display: flex;">
            <div class="rating-star-cont">
                <img src="${contextPath}/resources/static/star-fill.svg" alt="">
            </div>
            ...
            <div class="rating-star-cont">
                <img src="${contextPath}/resources/static/star-transparent.svg" alt="">
            </div>
        </div>
    ...
    <div class="card-main" data="${good.id}">
        <div class="good-images-cont">
            ...
            <div class="good-images-list">
                <c:forEach items="${good.goodImages}" var="goodImage" varStatus="status">
                    <div class="good-image-small c-pointer ${status.index == 1 ? "good-image-small-active" : ""}">
                        <img src="${goodImage.imageSrc}" alt="${good.name}">
                    </div>
                </c:forEach>
            </div>

            <div class="active-image c-pointer">
        ...
        <div class="good-details">
            <div class="good-price">
                <div style="font-size: 28px; margin-right: 20px;">
                    <!-- Current price -->
                    <span class="good-price__value">${good.price} ₽</span>
                </div>
                <div>
                    <c:if test="${good.priceBeforeDiscount != null}">
                        <div class="good-price-discount">
                            <span style="font-size: 16px; color: #8b8b8b;">${good.priceBeforeDiscount} ₽</span>
                            <span style="font-size: 20px; color: purple;  margin-left: 8px;">-${ Math.round(100 * (1 - good.price / good.priceBeforeDiscount)) }%</span>
                        </div>
                    </c:if>
                </div>
            </div>

            <!-- if currentGood.getGoodOpsions().size() != 0 else hide -->

            <c:if test="${good.goodOptions != null && good.goodOptions.size() != 0}">
                <div class="good-variations">
                    <div class="good-variations__items">
                        <c:forEach var="goodOption" items="${good.goodOptions}">
                            <div class="good-variations__item c-pointer">${goodOption.name}</div>
                        </c:forEach>
                    </div>
                </div>
            </c:if>
            ...
            <div class="other-good-info">
                <div>Продавец:
                    <a href="/store/showStore?storeId=${good.store.id}" target="_blanc">${good.store.name}</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <c:choose>
        <c:when test="${good.reviews.size() == 0}">
            <h4>Отзывов на этот товар пока нет. Станьте первым!</h4>
        </c:when>
        
        <c:otherwise>
            <div class="reviews">
                <%for ( int i = 0; i < good.getReviews().size(); i++){ %>
                    <div class="review" style="display: flex;">
                        <div class="user-avatar">
                            <img src="${contextPath}/resources/static/avatar-person.svg" alt="#">
                        </div>

                        <div>
                            <div class="review-firstName"><%= good.getReviews().get(i).getUser().getFirstName() %></div>

                            <div class="stars-cont">
                                <% int lastIndex = 0; %>

                                <% for (int j = 0; j < good.getReviews().get(i).getMark(); j++){%>
                                    <div class="star-cont"><img src="${contextPath}/resources/static/star-fill.svg" alt="#"></div>
                                    <% lastIndex = j+1; %>
                                <%}%>

                                <% for (int j = lastIndex; j < 5; j++){%>
                                    <div class="star-cont"><img src="${contextPath}/resources/static/star-transparent.svg" alt="#"></div>
                                <%}%>
                            </div>

                            <div class="review-text">
                                <%= good.getReviews().get(i).getText() %>
                            </div>
                        </div>
                    </div>
                <%}%>
...
<div class="good-gallery hide">
    <div class="good-gallery-wrapper">
        <div class="good-gallery__img-previews">
            <c:forEach items="${good.goodImages}" var="goodImage">
                <div class="good-gallery__img-preview good-image-small-active c-pointer">
                    <img src="${goodImage.imageSrc}" alt="${good.name}">
                </div>
            </c:forEach>
        </div>

        <div class="good-gallery__current">
            <div class="gallery-ctrl-btn ctrl-btn-left c-pointer">
                <svg width="24" height="24" fill="none"
                     xmlns="http://www.w3.org/2000/svg" class="_1nZS" style="transform: rotate(180deg);">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M17.586 12L8.293 2.707a1 1 0 011.414-1.414l10 10a1 1 0 010 1.414l-10 10a1 1 0 01-1.414-1.414L17.586 12z"
                          fill="currentColor"></path>
                </svg>
            </div>

            <div class="good-selected-image">
                <img src="${good.goodImages.get(0).imageSrc}" alt="">
            </div>

            <div class="gallery-ctrl-btn ctrl-btn-right c-pointer">
                <svg width="24" height="24" fill="none" xmlns="http://www.w3.org/2000/svg" class="_1nZS">
                    <path fill-rule="evenodd" clip-rule="evenodd"
                          d="M17.586 12L8.293 2.707a1 1 0 011.414-1.414l10 10a1 1 0 010 1.414l-10 10a1 1 0 01-1.414-1.414L17.586 12z"
                          fill="currentColor">
                    </path>
                </svg>
            </div>
        ...
        <div class="close-good-gallery main-menu-close-btn"></div>
    </div>
